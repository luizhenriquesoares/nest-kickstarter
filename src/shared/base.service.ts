...
export abstract class BaseService<T extends Typegoose> {
  ...
  async findById(id: string): Promise<InstanceType<T>> {
    return this._model.findById(this.toObject(id)).exec();
  }
  ...
  async delete(id: string): Promise<InstanceType<T>> {
    return this._model.findByIdAndDelete(this.toObject(id)).exec();
  }

  async update(id: string, item: InstanceType<T>): Promise<InstanceType<T>> {
    return this._model.findByIdAndUpdate(this.toObject(id), item, { new: true }).exec();
  }
  ...
  private toObject(id: string): Types.ObjectId {
